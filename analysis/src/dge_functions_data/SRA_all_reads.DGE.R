...
#we need this functionin the shared functions
...
for(tissue in c(tissue1, tissue2)){
    tissue_exp_data[[tissue]] <- read_data_of_one_tissue(paste0(project, ".", tissue, ".exon.merged.txt"), paste0(project, ".", tissue, ".intron.merged.txt"), normalization=F, tissue=tissue)
}
...
dev.off()
logFC_thres <- 0.5
visualize_cpm_fdr(dge_out[[intron_cmp_name]]$edger_out$table, dge_out[[exon_cmp_name]]$edger_out$table, 0, "edgeR.logCPM0", logFC_thres=logFC_thres)
visualize_cpm_fdr(dge_out[[intron_cmp_name]]$edger_out$table, dge_out[[exon_cmp_name]]$edger_out$table, 5, "edgeR.logCPM5", logFC_thres=logFC_thres)

visualize_cpm_fdr(dge_out[[intron_cmp_name]]$deseq2_out, dge_out[[exon_cmp_name]]$deseq2_out, 0, "DESeq2.logCPM0", logFC_thres=logFC_thres)
visualize_cpm_fdr(dge_out[[intron_cmp_name]]$deseq2_out, dge_out[[exon_cmp_name]]$deseq2_out, 5, "DESeq2.logCPM5", logFC_thres=logFC_thres)
